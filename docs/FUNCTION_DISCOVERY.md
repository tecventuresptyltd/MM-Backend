...
| `cancelJoinRequest` | Removes the caller’s pending request. |
...
| `promoteClanMember` | Raises a member’s rank (never to leader). |
| `demoteClanMember` | Lowers a member’s rank. |
...
| `getClanLeaderboard` | Returns `{ clanId, name, badge, type, members, totalTrophies }` for the top clans ordered by trophies. |
...
| Function | Purpose | Trigger |
| --- | --- | --- |
| `getGlobalLeaderboard` | Reads the cached `/GlobalLeaderboard/{metric}` doc (top 100 with clan info) rebuilt every 5 minutes. | HTTPS `onCall` |
| `getMyLeaderboardRank` | Returns the caller's exact rank/value for a metric using a COUNT aggregate (1 read). | HTTPS `onCall` |
...
| `removeFriends` | Removes one or more confirmed friends for the caller and target(s), updating `/Social/Friends` plus `friendsCount`. | HTTPS `onCall` |
