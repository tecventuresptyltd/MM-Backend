...
### Recent Changes
*   **2026-08-13:** Rebuilt Daily Rewards as a 7-day, player-anchored ladder with a server-derived day boundary (`graceDays: 1` forgives one missed day). Fixed a bug that made every claim fail: the old `claimDailyReward` granted inventory *after* writing, violating Firestore's read-before-write rule. Added `prepareInventoryGrants`/`applyInventoryGrants`, the shared `rewardBundle` resolver, and `npm run tools:validate-daily-rewards`. See [**Daily Rewards**](./DAILY_REWARDS.md) for the full plain-English guide.
...
* **Schema:** `/Players/{uid}/Social/*`, `/GlobalLeaderboard/{metric}` (optional cache), `/ClanLeaderboard/snapshot`, `/Usernames/{displayNameLower}`, plus the RTDB presence paths documented in [**FIRESTORE_SCHEMA.md**](./FIRESTORE_SCHEMA.md#playersuid-socialprofile-singleton).
