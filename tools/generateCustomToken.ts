/**
 * SAFE ADMIN TOOL: Generate Custom Token for Emergency Login
 * 
 * This script ONLY reads user data and generates a custom token.
 * It does NOT modify, delete, or change anything in the account.
 * 
 * Usage:
 * 1. For PRODUCTION: Update the service account path and projectId below
 * 2. Run: TS_NODE_PROJECT=../tsconfig.tools.json node -r ts-node/register ./tools/generateCustomToken.ts
 * 
 * The custom token can be used in Unity with:
...
try {
    ...
    console.error(`  1. The production service account file (backend-production-mystic-motors-prod.json)`);
...
async function generateCustomToken(): Promise<void> {
    console.log('🔐 Custom Token Generator - SAFE READ-ONLY MODE');
    console.log('================================================\n');
    ...
    try {
        ...
        const userRecord = await auth.getUser(TARGET_USER_ID);
        ...
        if (userDoc.exists) {
            ...
            console.log('⚠️  User document not found in Firestore (might be a new user)\n');
        }
